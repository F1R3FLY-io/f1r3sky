...
import {ThreadDraft} from "#/view/com/composer/state/composer.ts";
...
async function getBotAgentTeamUri(agent: BskyAgent, did: string): Promise<string | null> {
    try {
        const res = await agent.com.atproto.repo.getRecord({
            repo: did,
...
function removeMentionFacets(text: string, facets?: any[]): string {
    ...
    return dec.decode(new Uint8Array(out)).replace(/\s{2,}/g, ' ').trim()
}
...
function buildReplyToFromAppViewThread(appViewThreadItems: any[]): FireskyReply | null {
...
export async function sendBotInfo(
    ...
    for (const did of mentionedDids) {
        ...
        const phloLimit = Amount.tryFrom(BigInt(500000));

        await embers.agentsTeams.runOnFiresky(agentTeamUri as any, prompt, phloLimit, replyTo ?? undefined)

        console.log('Would run bot on Firesky:', {did, agentTeamUri, prompt, replyTo, postUri})
        return true
    }
